...
def generate_launch_description():
    pkg_dir = get_package_share_directory('myagv_plus_navigation2')
    workspace_dir = os.path.abspath(os.path.join(pkg_dir, '..', '..', '..', '..'))
    map_pkg_dir = os.path.join(workspace_dir, 'src', 'myagv_plus_navigation2')

    use_sim_time = LaunchConfiguration('use_sim_time', default='false')
    use_rviz = LaunchConfiguration('use_rviz', default='true')
    map_dir = LaunchConfiguration(
        'map',
        default=os.path.join(map_pkg_dir, 'map', 'map.yaml'))
