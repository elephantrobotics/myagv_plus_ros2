...
class AutomaticParkingVision(Node):
    ...
    def __init__(self):
        ...
        self.ParkingSequence = Enum(
            'ParkingSequence',
            'waiting marker_warmup search align align_y align_yaw measure parking done'
        )
    ...
    def execute_callback(self, goal_handle):
        ...
        self.reset_parking_state(goal_handle.request.marker_id)
        if self.target_marker_id < 0:
            self.get_logger().info('Parking will use any visible ArUco marker.')
        if goal_handle.request.expect_region:
            self.get_logger().warn(
                f'expect_region={goal_handle.request.expect_region} ignored: '
                'lite has no OCR region check.')
    ...
    def reset_parking_state(self, marker_id):
        self._done = False
    ...
    def _run(self):
        ...
        if self.current_state == self.ParkingSequence.marker_warmup:
        ...
        elif self.current_state == self.ParkingSequence.search:
