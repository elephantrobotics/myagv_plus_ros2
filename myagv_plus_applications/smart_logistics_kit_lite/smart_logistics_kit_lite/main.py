...
class LogisticsRouteMissionLite(Node):
    def __init__(self):
        super().__init__('logistics_route_mission_lite')

        self.declare_parameter('language', 'zh')
        self.language = self.get_parameter('language').value.strip().lower()

        self.default_city_to_waypoint = {  # 二维码城市到导航点位默认值；Default QR city to navigation waypoint.
            '北京市': 'A',    # Beijing
            '大连市': 'A',    # Dalian
            '上海市': 'A',    # Shanghai
            '南京市': 'B',    # Nanjing
        ...
        self.waypoints_file = self.locate_waypoints_file()  # 导航点位与城市映射文件路径；Waypoint and city mapping YAML path.
        ...
        self.bridge_timeout = 600.0  # route_bridge 导航结果最长等待时间，单位秒；Max wait for route_bridge result, in seconds.
        self.refiner_timeout = 30.0  # 最终姿态精修最长等待时间，单位秒；Max wait for final pose refinement, in seconds.
        self.parking_timeout = 180.0  # 泊车动作结果最长等待时间，单位秒；Max wait for parking result, in seconds.
        ...
        self.city_to_waypoint = self.load_city_to_waypoint(self.waypoints_file)
        ...
        self.get_logger().info('Logistics lite node started. Checking subsystems before departure.')
    ...
    def locate_waypoints_file(self):
        share = Path(get_package_share_directory(PACKAGE_NAME)).resolve()
        source = (share.parents[3] / 'src' / 'myagv_plus_applications' /
                  PACKAGE_NAME / 'scripts' / 'waypoints.yaml')
        if source.is_file():
            return str(source)
        return str(share / 'scripts' / 'waypoints.yaml')
    ...
    def run_once(self):
        ...
        if not self.navigate_to_waypoint(waypoint):
            ...
            return False
        if not self.run_parking(waypoint, 'destination'):
    ...
    def wait_for_bridge_result(self):
        ...
        while rclpy.ok() and time.monotonic() < deadline:
            ...
            time.sleep(0.1)
        self.get_logger().warn(f'No final route_bridge feedback within {self.bridge_timeout} s.')
        self.cancel_route_bridge()
        return False

    def wait_for_refiner(self, start_seq):
        deadline = time.monotonic() + self.refiner_timeout
        saw_running = False
        sequence = start_seq
        while rclpy.ok() and time.monotonic() < deadline:
            ...
            for sequence, status in updates:
                if status == 'running':
                    ...
                    return False
            time.sleep(0.1)
        self.get_logger().warn(f'No final pose refinement success within {self.refiner_timeout} s.')
        return False

    def cancel_route_bridge(self):
        self.route_cancel_pub.publish(Bool(data=True))
        self.get_logger().warn('Sent route_bridge cancel.')

    def run_parking(self, waypoint, label):
        self.get_logger().info(
        ...
        if not self.wait_future(result_future, self.parking_timeout):
    ...
    def city_label(self, city):
        if self.language == 'en':
            return self.city_to_english.get(city, city)
        return city

    def resolve_waypoint(self, text):
        city = next((city for city in self.city_to_waypoint if city in text), None)
        waypoint = self.city_to_waypoint.get(city)
        if waypoint in self.waypoints:
            self.get_logger().info(
                f'Resolved QR text "{text}" -> city "{self.city_label(city)}" -> waypoint {waypoint}')
            return waypoint
    ...
    def warn_if_pickup_is_destination(self):
        for city, waypoint in self.city_to_waypoint.items():
            if waypoint == self.pickup_waypoint:
                self.get_logger().warn(
                    f'City {self.city_label(city)} maps to pickup waypoint {self.pickup_waypoint}. '
                    'Update city_to_waypoint if the pickup point is not a destination.')
    ...
    def load_city_to_waypoint(self, waypoints_file):
        data = yaml.safe_load(Path(waypoints_file).read_text(encoding='utf-8')) or {}
        loaded = data.get('city_to_waypoint') or {}
        if not loaded:
            self.get_logger().warn(
                'No city_to_waypoint field in waypoints.yaml. Using built-in defaults.')
            return dict(self.default_city_to_waypoint)
        mapping = {}
        for city, waypoint in loaded.items():
            mapping[str(city).strip()] = str(waypoint).strip().upper()
        display = {self.city_label(city): waypoint for city, waypoint in mapping.items()}
        self.get_logger().info(f'Loaded city mapping from {waypoints_file}: {display}')
        return mapping
...
def main(args=None):
    ...
    node = None
    try:
        node = LogisticsRouteMissionLite()
        rclpy.spin(node)
