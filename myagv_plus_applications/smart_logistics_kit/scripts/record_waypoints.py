...
class WaypointRecorder(Node):
    VALID_WAYPOINT_KEYS = set(string.ascii_uppercase)
    PRIORITY_WAYPOINT_KEYS = ('O', 'P')

    def __init__(self):
        super().__init__('waypoint_recorder')
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.yaml_path = str(Path(__file__).resolve().parent / 'waypoints.yaml')
        self.waypoints = {}
        self._load_existing()
    ...
    def _ordered_waypoints(self):
        ordered = {}
        for key in self.PRIORITY_WAYPOINT_KEYS:
            if key in self.waypoints:
                ordered[key] = self.waypoints[key]
        for key in sorted(k for k in self.waypoints if k not in self.PRIORITY_WAYPOINT_KEYS):
            ordered[key] = self.waypoints[key]
        return ordered

    def _save(self):
        self.waypoints = self._ordered_waypoints()
        try:
            with open(self.yaml_path, 'r') as f:
                data = yaml.safe_load(f) or {}
        except FileNotFoundError:
    ...
    def run(self):
        ...
        self.get_logger().info('Enter A-Z to record. O=pickup, P=reserved charging dock, q=quit.')
        while rclpy.ok():
            try:
            ...
            if cmd == 'Q':
                break
            if len(cmd) == 1 and cmd in self.VALID_WAYPOINT_KEYS:
                ...
                self.get_logger().warn('Use one letter from A-Z, or q.')
