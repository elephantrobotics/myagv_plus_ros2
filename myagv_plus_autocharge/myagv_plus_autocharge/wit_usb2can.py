...
class SerialCANParser:
    ...
    并提供实时的速度和传感器信息接口。
    """
    def __init__(self, serial_port='/dev/ttyCH341USB1', baudrate=9600, timeout=1, debug=True, register_signal_handlers=False):
        """
        参数:
    ...
    def open_serial(self):
        """打开串口"""
        if serial is None:
            raise RuntimeError('Missing Python dependency: pyserial. Install python3-serial first.')
        try:
    ...
    def read_serial_data_once(self):
        ...
        if self.debug:
            ...
            print(f"帧ID: 0x{can_frame_id:X}")
        self.buffer.clear()

        if can_frame_id == 0x182 and data_length == 0x08: # 根据can帧id进行判断
            # 如果帧ID为0x182,校验通过,进行数据赋值
            return self.parse_can_data(data)
        return None
    ...
    def read_serial_response(self):
        ...
        while True:
            ...
            # 超时机制，防止死循环
            if len(response) > 100 or time.monotonic() > deadline:
                break
    ...
    def start(self):
        ...
        try:
            ...
            # 开始读取数据
            while True:
                x_speed, z_speed, which_mode, infrared_bits = self.read_serial_data()
...
if __name__ == '__main__':
    parser = SerialCANParser('/dev/ttyCH341USB1', 9600, 1, True, register_signal_handlers=True)
    parser.start()
